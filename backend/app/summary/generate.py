...
from backend.app.llm_client import ParitokLLMClient, compress_for_prompt
...
def _build_prompt(query: str, papers: list[dict]) -> tuple[list[dict], int, int]:
    ...
    compressed_stuffed, original_tokens, compressed_tokens = compress_for_prompt(stuffed, query)
    messages = [
        {"role": "system", "content": (
...
def generate_sourced_summary(
    client: ParitokLLMClient, query: str, papers: list[dict], *, low_confidence: bool = False, sparse_coverage: bool = False, on_stage: Callable[[str, dict], None] | None = None,
) -> SourcedSummary:
    if not papers:
